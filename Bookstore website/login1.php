...
<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.0/jquery.min.js'></script>
 <script src='https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.7/js/bootstrap.min.js'></script>
    ...
    <?php if(isset($_POST['signup'])){
        ...
        $sql= "SELECT * FROM customer WHERE email = '$c_email' LIMIT 1" ;
        $check_user=mysqli_query($conn,$sql);
        $count_email = mysqli_num_rows($check_user);
        if($count_email > 0){
         echo "<script>alert('User is already Register ')</script>";
         exit();
       }
       else{
        $sql = "INSERT INTO customer (`cust_ip`, `name`, `email`, `password`, `phone`) VALUES ('$ip','$c_name','$c_email','$c_pass','$c_phone')";
        $result = mysqli_query($conn, $sql);
        if($result){
            echo "<script>alert('Registration Successful Please Do Login')</script>";
